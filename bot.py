...
# -----------------------
# Универсальный Обработчик Кнопок (Callbacks)
# -----------------------
...
🎯 <code>/darts [ставка]</code>"""
    ...
    # --------------- Запуск игр в реальном времени с кнопок ---------------
    elif query.data in ["play_bowling_now", "play_slots_now", "play_dice_now", "play_darts_now"]:
        ...
        # Математика и множители
        multiplier = 0.0
        if emoji == "🎳":
            # 1 - мимо (0 кегель), 6 - страйк!
            mults = {1: 0.0, 2: 0.5, 3: 1.0, 4: 1.5, 5: 2.0, 6: 3.5}
            multiplier = mults.get(val, 1.0)
        elif emoji == "🎰":
            if val in [1, 22, 43, 64]: multiplier = 15.0  # Джекпоты
            elif val in [16, 32, 48]: multiplier = 5.0    # 3 одинаковых
            elif val in [2, 3, 4, 10]: multiplier = 1.5
        elif emoji == "🎲":
            # 1,2,3 - проигрыш дилеру, 4,5,6 - победа
...
*пусто*"""
    ...
    # --------------- Заглушки ---------------
    elif query.data in ("btn_fast", "btn_modes"):
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not match:
        # Также ищем варианты на русском без слэша (например "боулинг 100", "слоты 50")
    ...
    if match:
        ...
        if cmd in ["buckshot", "pp", "рулетка"]:
            # 1 патрон в барабане на 6 гнезд
        ...
        res_text = f"🎉 <b>Выигрыш {win_sum} ncoin!</b>" if win_sum > 0 else "😔 <b>Ставка не сыграла.</b>"
