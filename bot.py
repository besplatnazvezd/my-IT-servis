...
def format_number(val: int | float) -> str:
    """Форматирует большие числа в сокращенный вид (1000 -> 1k, 1000000 -> 1kk)."""
    val = int(val)
    if val >= 1_000_000_000:  # Миллиарды
        return f"{val / 1_000_000_000:.2f}b"
    elif val >= 1_000_000:  # Миллионы
        return f"{val / 1_000_000:.2f}kk"
    elif val >= 1_000:  # Тысячи
...
async def db_get_top_users(order_by: str, limit: int = 10) -> List[Dict[str, Any]]:
    """Получает топ пользователей по указанному полю (ncoin или nmp)."""
    try:
        async with httpx.AsyncClient() as client:
            r = await client.get(
                f"{SUPABASE_URL}users?order={order_by}.desc&limit={limit}",
                headers=HEADERS,
                timeout=6.0,
            )
            r.raise_for_status()
...
# Меню «Быстрые» (По умолчанию при "Играть" или /game)
...
# Специальные подклавиатуры игр с быстрыми действиями
...
async def _send_game_menu(
    ...
    keyboard_type: str = "fast" # "fast" или "modes"
) -> None:
    text = (
...
async def game_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработчик команды /game, показывающий основное меню игр."""
    user = update.effective_user
...
async def top_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработчик команды /top, показывающий топ игроков."""
    user = update.effective_user
    ...
    for i, p in enumerate(top_ncoin):
        ncoin_leaderboard.append(
            f"{i+1}. 🏆 <a href='tg://user?id={p['tg_id']}'>{p['username']}</a> | <code>{format_number(p['ncoin'])} nCoin</code>"
        )

    nmp_leaderboard = []
    for i, p in enumerate(top_nmp):
        nmp_leaderboard.append(
            f"{i+1}. ⭐️ <a href='tg://user?id={p['tg_id']}'>{p['username']}</a> | <code>{format_number(p['nmp'])} nMP</code>"
        )

    text = (
        "<b>🏆 МИРОВОЙ ТОП ПО ИГРОКАМ • ЗА ВСЕ ВРЕМЯ</b>\n\n"
        "💎 <b>ТОП по nCoin:</b>\n"
...
<b>Пример:</b> боулинг 100"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_bowling_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_bowling_now"), parse_mode="HTML"
            )
...
<b>Пример:</b> слоты 100"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_slots_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_slots_now"), parse_mode="HTML"
            )
...
🎲 <code>/dice [ставка]</code>"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_dice_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_dice_now"), parse_mode="HTML"
            )
...
🎯 <code>/darts [ставка]</code>"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_darts_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_darts_now"), parse_mode="HTML"
            )
...
<b>Пример:</b> рр 100"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_buckshot_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_buckshot_now"), parse_mode="HTML"
            )
...
🪙 <code>/coin [ставка] [орел/решка]</code>"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_coin_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_coin_now"), parse_mode="HTML"
            )
...
🎱 <code>/roulette [ставка] [к/ч/число]</code>"""
        try:
            await query.edit_message_text(
                text=text, reply_markup=get_game_action_keyboard("play_roulette_now"), parse_mode="HTML"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=query.message.chat_id, text=text, reply_markup=get_game_action_keyboard("play_roulette_now"), parse_mode="HTML"
            )
...
*пусто*"""
    ...
    # --------------- Пустые заглушки для новых игр ---------------
        ...
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text="🚧 Этот режим игры сейчас находится в активной разработке!",
            parse_mode="HTML",
        )
...
# -----------------------
# Обработчик Текста (Игры по командам и Баланс)
# -----------------------
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    match = re.match(
        r"^(/(bowling|slots|dice|darts|buckshot|pp|coin|roulette)|(боулинг|слоты|кубик|дартс|рулетка|монета))\s*(\d+)?(?:\s+(орел|решка|к|ч|\d+))?",
        clean_text,
    )

    if match:
        cmd_group_1 = match.group(2)  # команда со слэшем
        cmd_group_3 = match.group(3)  # команда без слэша (русская)
        cmd = cmd_group_1 if cmd_group_1 else cmd_group_3

        bet_str = match.group(4)
        extra_param = match.group(5) # для coin (орел/решка) или roulette (к/ч/число)
        ...
        if balance < bet:
            await update.message.reply_text(
                "❌ <b>У вас недостаточно ncoin для этой ставки!</b>", parse_mode="HTML"
            )
            return

        balance -= bet
        await db_update(
            user_id, {"ncoin": balance, "games_played": user_data.get("games_played", 0) + 1}
        )

        # --- Игра: Русская Рулетка / Buckshot / PP ---
        ...
        # --- Игра: Орел и Решка (Coin) ---
            ...
            choice = "орел" if "орел" in (extra_param or "") or "heads" in (extra_param or "") else "решка"
        ...
        # --- Игра: Рулетка ---
        elif cmd in ["roulette", "рулетка"]:
            roulette_outcome_val = random.randint(0, 36) # 0 to 36
            roulette_outcome_color = "черный" if roulette_outcome_val % 2 == 0 and roulette_outcome_val != 0 else "красный" if roulette_outcome_val != 0 else "зеленый" # 0 зеленый, остальное чет/нечет
            
            win = 0
            multiplier = 0.0

            if extra_param:
                if extra_param in ["к", "красный"] and roulette_outcome_color == "красный":
                    multiplier = 2.0
                elif extra_param in ["ч", "черный"] and roulette_outcome_color == "черный":
                    multiplier = 2.0
                elif extra_param.isdigit() and int(extra_param) == roulette_outcome_val and roulette_outcome_val != 0:
                    multiplier = 35.0
                elif extra_param == "0" and roulette_outcome_val == 0:
                     multiplier = 35.0
            
            if multiplier > 0:
                win = int(bet * multiplier)
                await db_update(user_id, {"ncoin": balance + win})
                res = f"🎱 Выпало <b>{roulette_outcome_val} ({roulette_outcome_color})</b>! <b>Вы выиграли!</b>\nВыигрыш: <code>{win} ncoin</code>"
            else:
                await db_update(user_id, {"lost_ncoin": user_data.get("lost_ncoin", 0) + bet})
                res = f"🎱 Выпало <b>{roulette_outcome_val} ({roulette_outcome_color})</b>! Вы проиграли <code>{bet} ncoin</code>."
            await update.message.reply_text(res, parse_mode="HTML")
            return

        # --- Интерактивные Telegram-Dice Игры ---
        emoji = "🎲"
        if cmd in ["bowling", "боулинг"]:
            emoji = "🎳"
        elif cmd in ["slots", "слоты"]:
            emoji = "🎰"
        elif cmd in ["darts", "дартс"]:
            emoji = "🎯"
        ...
        await update.message.reply_text(
            f"{res_text}\n💰 <b>Баланс:</b> <code>{balance + win_sum} ncoin</code>",
            parse_mode="HTML",
        )
        return

    # 3. Изменение ставки по прямому вводу
    if user_states.get(user_id) == "awaiting_bet":
        ...
        await update.message.reply_text(
            ...
            reply_markup=get_fast_keyboard(), # Возвращаем на основную игровую клавиатуру
            parse_mode="HTML",
        )
...
async def test_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text="Тестовая клавиатура:", reply_markup=kb
    )
